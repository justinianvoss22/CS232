...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: runs programs whose starting locations are in the array starting at location <addr>")
                    continue
                ...
                # add new instruction for the letter R
    ...
    # New function
    # puts the CPU into batch mode and then runs the program
    def _run_batch(self, addr):

        # sets this attribute to true

        self._cpu = CPU(self._ram, calos.CalOS(), addr, self._debug, batchMode=True) # calls the function with batch mode being true
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
