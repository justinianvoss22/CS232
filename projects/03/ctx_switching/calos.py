...
class CalOS:
    ...
    def timer_isr(self):
        ...
        '''
        CalOS.current_proc.set_registers(self._cpu.get_registers()) # Saves the CPU registers into the PCB registers
        
        
        if (len(self._ready_q) != 0) :              # if the processes are not empty, it switches, using context switching
            self.context_switch() # otherwise, it just turns back the timer to keep on going
        
        self.reset_timer()        # resets the timer
    ...
    def context_switch(self):
        ...
        ''' 
        CalOS.next_proc = self._ready_q.pop()
        CalOS.current_proc.set_registers(self._cpu.get_registers())
        self._cpu.set_registers(CalOS.next_proc.get_registers()) # Loads the CPU registers to the new process' registers
        self.add_to_ready_q(CalOS.current_proc) # adds to the ready queue of processes
        CalOS.next_proc.set_state(PCB.RUNNING) # sets the state of the process as RUNNING
        CalOS.current_proc = CalOS.next_proc
    ...
    def run(self):
        ...
        '''
        while (len(self._ready_q) != 0) : # while the ready queue is not empty
           CalOS.current_proc = self._ready_q.pop() # sets the registers to the current result that is popped off the ready queue
           self.reset_timer() # resets the timer
           self._cpu.set_registers(CalOS.current_proc.get_registers())  # Sets the registers to the current PCB
           self._cpu.run_process() # Runs the current process
           CalOS.current_proc.set_state(PCB.DONE)  # Sets the current state to DONE, signifying that it has been run
